...
class SurfaceGenerator(ExternalCode):
    geo_script = 'aero_jig.geo'

    jig_mesh = 'aero_jig.msh'


    def __init__(self, n_sec, ngeom, ngeom_unique, network_info, ref_airfoil_file):
        ...
        self.add_output('apoints_coord', val=np.zeros((self.ngeom, 3)))

        #Coordinates of the unique jig shape aerodynamic points
        self.add_output('apoints_coord_unique', val=np.zeros((self.ngeom_unique, 3)))
        ...
        if sys.platform == 'win32':
            ...
            self.options['command'] = ['gmsh', self.input_filepath, '-2', '-format', 'msh2', '-o', self.output_filepath]
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        super(SurfaceGenerator, self).solve_nonlinear(params, unknowns, resids)
    ...
    def create_gmsh_script(self, params, ref_airfoil):
            ...
            for i in range(sec_num):
                ...
                for j in range(foil_num):
                    if (j == 0 or j == foil_num - 1):
                        ...
                        f.write('Point('+str(j+1+2*i*len(upper_coord))+') = {'+str(c[i]*float(upper_coord[j][0]))+', 0, '+str(c[i]*y_u+delta)+'};\n')
                    else:
                        y_u = camc[i]/camc_ref*camber_ref[j] + tc[i]/tc_ref*half_thick_ref[j]
                        f.write('Point('+str(j+1+2*i*len(upper_coord))+') = {'+str(c[i]*float(upper_coord[j][0]))+', 0, '+str(c[i]*y_u)+'};\n')

                #Write lower surface coordinates
                offset = len(upper_coord)
                for j in range(foil_num):
                    y_l = camc[i]/camc_ref*camber_ref[j] - tc[i]/tc_ref*half_thick_ref[j]
                    f.write('Point('+str(j+1+offset+2*i*len(lower_coord))+') = {'+str(c[i]*float(lower_coord[j][0]))+', 0, '+str(c[i]*y_l)+'};\n')
        ...
        #Translation of the airfoil sections
            for i in range(sec_num):
                f.write('Translate {'+str(xle[i])+', '+str(yle[i])+', '+str(zle[i])+'} {Duplicata{Line{'+str(2*i+1)+', '+str(2*i+2)+'};}}\n\n')
...
#            bump = (1-math.cos(math.pi/rows))/(2*math.sin(math.pi/(2*rows)))
        ...
        #Transform airfoil splines into transfinite lines
            for i in range(2*sec_num):
                f.write('Transfinite Line {'+str(2*sec_num+i+1)+'} = ('+str(rows+1)+');\n')
            f.write('\n')
