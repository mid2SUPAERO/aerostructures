...
class AeroProblemDimensions:
    ...
    aero_template_lo = 'aero_template_lo.wgs'
    ...
    def __init__(self, fidelity):
        ...
        if self.fidelity == 'high':
            ...
            self.aero_dimensions = self.get_aero_dimensions_lo()
    ...
    def get_aero_dimensions_lo(self):

        apoints_coord = []
        network_info = []

        #Write the aerodynamic grid points coordinates into a list
        with open(self.aero_template_lo) as f:
            lines = f.readlines()
            lines = [i.split() for i in lines]
